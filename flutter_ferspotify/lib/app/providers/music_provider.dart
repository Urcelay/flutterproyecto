...
class MusicProvider {
  ...
  static Future<List<SongModel>> getSong({
    int? category,
    int? artist,
    int? album,
  }) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print("🟢 [MusicProvider] Parseando lista de canciones (${data.length})...");
...
static Future<List<SongModel>> getFavorites(String token) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      print("🟢 [MusicProvider] Parseando lista de canciones favoritas (${data.length})...");
      return data.map((song) => SongModel.fromJson(song)).toList();
    } else {
...
static Future<FavoriteResponseModel?> toggleFavorite(
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      print("🟢 [MusicProvider] Parseando respuesta de favoritos...");
...
static Future<List<SongModel>> getTopSongs() async {
...
static Future<List<SongModel>> searchSongs(String query) async {
  final url = Uri.parse('$BASE_URL/music/search')
  ...
  try {
    ...
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      print("🟢 [MusicProvider] Resultados de búsqueda: ${data.length} canciones");
      return data.map((song) => SongModel.fromJson(song)).toList();
    } else {
...
/// NO requiere autenticación.
static Future<LikeResponseModel?> likeSong(int musicId) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      return LikeResponseModel.fromJson(data);
    } else {
...
/// No requiere autenticación.
static Future<PlayResponseModel?> playSong(int musicId) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      return PlayResponseModel.fromJson(data);
    } else {
