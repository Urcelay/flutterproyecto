...
class PlaylistSongProvider {
  ...
  /// Endpoint: POST /api/lista/{id}/add
  static Future<SimpleResponse?> addSongsToPlaylist(
      String token, int playlistId, List<int> musicIds) async {
  ...
  /// Requiere autenticación.
  /// Endpoint: GET /api/lista/{id}/songs
  static Future<PlaylistSongsResponseModel?> getPlaylistSongs(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        print("🟢 [PlaylistSongProvider] Playlist y canciones obtenidas.");
  ...
  /// [musicId] = ID de la canción a eliminar
  /// Devuelve un [SimpleResponseModel].
  static Future<SimpleResponse?> removeSongFromPlaylist(
      String token, int playlistId, int musicId) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        print("🟢 [PlaylistSongProvider] Canción eliminada de la playlist.");
        return SimpleResponse.fromJson(data);
      } else {
